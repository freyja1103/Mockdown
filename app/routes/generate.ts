...
export async function generate(c: Context) {
  ...
  if (!baseUrl) {
    return jsonError('AI_BASE_URL not configured', 500);
  }
  
  const model = process.env.AI_MODEL;
  if (!model) {
    return jsonError('AI_MODEL not configured', 500);
  }

  const apiKey = process.env.AI_API_KEY;
  if (!apiKey) {
    return jsonError('AI_API_KEY not configured', 500);
  }
  ...
  if (typeof prompt !== 'string' || prompt.length === 0 || prompt.length > MAX_PROMPT_LENGTH) {
    return jsonError(`Prompt must be 1-${MAX_PROMPT_LENGTH} characters`, 400);
  }
  if (!Number.isInteger(width) || !Number.isInteger(height) || (width as number) < 1 || (width as number) > 200 || (height as number) < 1 || (height as number) > 100) {
    return jsonError('Dimensions out of range (max 200×100)', 400);
  }
  if (existingContent != null && (typeof existingContent !== 'string' || existingContent.length > MAX_EXISTING_CONTENT)) {
    return jsonError('Existing content too large', 400);
  }
